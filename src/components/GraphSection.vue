...
export default {
  ...
  data() {
    return {
      ...
      selectedTicker: this.tickerToDraw,
    };
  },

  props: ["tickerToDraw", "graphElements"],
  emits: ["closeGraph"],
  watch: {
    normalizedGraph() {
      if (this.normalizedGraph.length > 4) {
        this.calculateMaxGraphElements();
        this.calculateGraphElementWidth();
        console.log("check");
      }
      while (this.graphElements.length > this.maxGraphElements) {
        this.graph = this.graph.slice(3, -1);
      }
      console.log("check");
    },
    tickerToDraw() {
      this.graph = this.graphElements;
    },
  },
